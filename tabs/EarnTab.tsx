  ...
  if (stats.knowledgePoints === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center space-y-6 animate-fadeIn py-12">
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-700 underline decoration-indigo-400">{t('earn.locked')}</h2>
          <p className="text-slate-500 mt-2 px-6 text-sm">
            {t('earn.lockHint')}
          </p>
        </div>
        <Button onClick={() => setActiveTab('school')} variant="academy">{t('earn.goToSchool')}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="flex justify-between items-end mb-2">
        <h2 className="text-xl font-black text-slate-800">{t('earn.title')}</h2>
        <div className="text-xs md:text-sm font-bold bg-slate-100 px-2 py-1 rounded-lg text-slate-500 flex items-center gap-1">
          <Calendar size={14} />
          {t('earn.dailyCount', { generated: dailyTasksGenerated, max: MAX_DAILY_TASKS })}
        </div>
      </div>
      ...
      {dailyTasksGenerated < MAX_DAILY_TASKS ? (
        <Button variant="magic" onClick={handleMagicMission} disabled={isMissionLoading}>
          ...
          {isMissionLoading ? t('earn.generating') : t('earn.newChallenge')}
        </Button>
      ) : (
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center text-orange-500 font-black text-xs mt-1">
                  <DollarSign size={14} strokeWidth={3} /> {t('earn.coinReward', { n: mission.reward })}
                </div>
              </div>
            </div>
            {!mission.completed ? (
